...
from dify_plugin.entities.model.llm import (
    LLMResult, LLMMode, LLMResultChunk, LLMResultChunkDelta,
)
from dify_plugin.entities.model.message import (
    PromptMessage,
    PromptMessageTool, AssistantPromptMessage, SystemPromptMessage, UserPromptMessage, PromptMessageRole,
    PromptMessageContentType, TextPromptMessageContent, ImagePromptMessageContent, ToolPromptMessage,
...
class ZhinaoOpenAILargeLanguageModel(_CommonOpenAI, LargeLanguageModel):
    """
    ...
    def _handle_generate_stream_response(
        ...
        for chunk in response:
            ...
            if delta.finish_reason is None and (delta.text is None or delta.text == ''):
                has_finish_reason = delta.finish_reason is not None and delta.finish_reason != ""
            if not has_finish_reason is None and (delta.text is None or delta.text == ''):
                continue
            # transform assistant message to prompt message
            text = delta.text or ""
    ...
    def _chat_generate(
        ...
        if model.startswith(("o1", "o3", "openai/o1", "openai/o3")):
            if "max_tokens" in model_parameters:
                model_parameters["max_completion_tokens"] = model_parameters["max_tokens"]
                del model_parameters["max_tokens"]

            if re.match(r"^o1-(?:mini|preview|\d{4}-\d{2}-\d{2})$", model):
                if stream:
    ...
    def _num_tokens_from_messages(
        ...
        if model == "chatgpt-4o-latest" or model.startswith(
                ("o1", "o3", "openai/o1", "openai/o3")):
            model = "gpt-4o"
        ...
        if model.startswith("gpt-3.5-turbo-0301"):
            ...
            tokens_per_name = -1
        elif model.startswith("gpt-3.5-turbo") or model.startswith("gpt-4") or model.startswith(
                ("o1", "o3", "openai/o1", "openai/o3")):
